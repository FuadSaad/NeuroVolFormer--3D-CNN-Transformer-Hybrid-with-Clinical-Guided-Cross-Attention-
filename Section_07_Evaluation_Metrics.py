...
def _load_or_generate_independent_baseline_probs(y_true: np.ndarray, num_classes: int = 4) -> np.ndarray:
    ...
    for p in candidate_paths:
        if os.path.exists(p):
            try:
                ...
                if base_probs.shape == (n_samples, num_classes):
                    print(f"📦 Loaded independent benchmark predictions from: {p}")
                    return base_probs
                elif len(base_probs) == n_samples:
                    print(f"📦 Loaded independent benchmark predictions from: {p}")
                    return base_probs
    ...
    print("   (Using empirical prevalence prior baseline for statistical demonstration.)")

    # Empirical class prevalence prior fallback
